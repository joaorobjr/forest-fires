...
Table \@ref(tab:base-variables) describes all variables contained in the data set that was analised in sections \@ref(data-cleaning) and \@ref(feature-engineering).

Table: (\#tab:base-variables) List of all variables to be analysed.
...
### origin
Origin informs the reason why the fire started and apparently appears to be an important observation for evaluation.It can be observed in figure \@ref(fig:origin-ratings).

### alert_date, alert_hour
...
### firstInterv_date, firstInterv_hour
...
### village_area, vegetation_area, farming_area, village_veget_area, total_area


### cause_type
This is the variable to be predicted by the model that will be chosen.It shows the four causes of the occurrences: intentional, natural, negligent and rekindling. They can be observed in the graphic \@ref(fig:causes-ratings).
...
ggplot(ffires.raw, aes(x = district)) +  geom_bar() + ggtitle("Distribution of forests fires by district") +
...
ggplot(ffires.raw, aes(x = cause_type)) + geom_bar(color = "black", fill = "light blue") + ggtitle("Distribution of causes of fires")+
...
ggplot(ffires.raw, aes(x = origin)) + geom_bar(fill = "blue") + ggtitle("Distribution of forests fires by origin across 2015")+
