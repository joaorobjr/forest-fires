...
It is an instance-based learning algorithm: it learns by analogy that is, they are based on the notion of similarity between cases. The K-nearest neighbors (KNN) algorithm uses 'resource similarity' to predict the values of new data points, which further means that the new data point will receive a value based on how close it corresponds to the points in the training set.
...
The package caret was used to create this model with the method "kknn".
The tunning parameters used for the model were kmax = 9, distance = 2 and kernel = optimal and the accuracy reached 58.92%. 
...
Performance: the ROC curve was plotted below and shows the relationship between sensitivity and specificity. Classifiers that provide curves closer to the upper left corner have the corresponding area under the larger curves (AUCs) and indicate better performance.Multi-class area under the curve: 73.82%
...
The package caret was used to create this model with the method "naive_bayes".
The tunning parameters used for the model were laplace = 0, usekernel = TRUE and adjust = 1 and the accuracy reached 56.06%.
...
the ROC curve was plotted below and shows the relationship between sensitivity and specificity. Classifiers that provide curves closer to the upper left corner have the corresponding area under the larger curves (AUCs) and indicate better performance. Multi-class area under the curve: 72.85%
...
```{r lr_model, eval=FALSE, echo=T, include=FALSE}
lrModel <- train(cause_type ~ ., data = trainSet, method = "multinom", trControl = ctrl,)
```

The package caret was used to create this model with the method "multinom".
The tunning parameter used for the model was decay = 0.1 and the accuracy reached 62.74%.
...
the ROC curve was plotted below and shows the relationship between sensitivity and specificity. Classifiers that provide curves closer to the upper left corner have the corresponding area under the larger curves (AUCs) and indicate better performance. Multi-class area under the curve: 81.1%
...
The package caret was used to create this model with the method "lda".
The accuracy reached 61.98%.
...
the ROC curve was plotted below and shows the relationship between sensitivity and specificity. Classifiers that provide curves closer to the upper left corner have the corresponding area under the larger curves (AUCs) and indicate better performance. Multi-class area under the curve: 81.05%
...
The package caret was used to create this model with the method "pda".
The tunning parameter used for the model lambda = 0.1 and the accuracy reached 62.04%.
...
The package caret was used to create this model with the method "rpart".
The final value used for the model was cp = 0.006843456. and the accuracy reached 61.49%.
...
the ROC curve was plotted below and shows the relationship between sensitivity and specificity. Classifiers that provide curves closer to the upper left corner have the corresponding area under the larger curves (AUCs) and indicate better performance. Multi-class area under the curve: 67.61%
...
The package caret was used to create this model with the method "treebag".
The accuracy reached 61.47%.
...
the ROC curve was plotted below and shows the relationship between sensitivity and specificity. Classifiers that provide curves closer to the upper left corner have the corresponding area under the larger curves (AUCs) and indicate better performance. Multi-class area under the curve: 74.79%
...
```{r nn_model, eval=FALSE, echo=T, include=FALSE}
nnModel <- train(cause_type ~ ., data = trainSet, method = "nnet", trControl = ctrl)
```

The package caret was used to create this model with the method "nnet".
The tunning parameters used for the model were size = 3 and decay = 0.1. and the accuracy reached was 60.69%.
...
the ROC curve was plotted below and shows the relationship between sensitivity and specificity. Classifiers that provide curves closer to the upper left corner have the corresponding area under the larger curves (AUCs) and indicate better performance. Multi-class area under the curve: 74.01%
...
The package caret was used to create this model with the method "svmLinear2".
The tunning parameter used for the model was cost = 1 and the accuracy reached was 60.92%.
...
the ROC curve was plotted below and shows the relationship between sensitivity and specificity. Classifiers that provide curves closer to the upper left corner have the corresponding area under the larger curves (AUCs) and indicate better performance. Multi-class area under the curve: 78.36%
...
The package caret was used to create this model with the method "svmRadial".
The tunning parameters used for the model were sigma = 0.01788098 and C = 0.5. and the accuracy reached was 58.16%.
...
the ROC curve was plotted below and shows the relationship between sensitivity and specificity. Classifiers that provide curves closer to the upper left corner have the corresponding area under the larger curves (AUCs) and indicate better performance.Multi-class area under the curve: 73.47%
...
The package caret was used to create this model with the method "svmPoly".
The tunning parameters used for the model were degree = 3, scale = 0.01 and C = 0.25 and the accuracy reached was 56.18%.
...
the ROC curve was plotted below and shows the relationship between sensitivity and specificity. Classifiers that provide curves closer to the upper left corner have the corresponding area under the larger curves (AUCs) and indicate better performance. Multi-class area under the curve: 73.8%
...
The package caret was used to create this model with the method "rf".
The final value used for the model was mtry = 22 and the accuracy reached was 63.30%.
...
the ROC curve was plotted below and shows the relationship between sensitivity and specificity. Classifiers that provide curves closer to the upper left corner have the corresponding area under the larger curves (AUCs) and indicate better performance. Multi-class area under the curve: 79.6%
...
XGBoost is an optimized distributed gradient boosting system designed to be highly efficient, flexible and portable. It implements machine learning algorithms under the Gradient Boosting framework. XGBoost provides a parallel tree boosting(also known as GBDT, GBM).
...
The package caret was used to create this model with the method "xgbTree".
The tunning parameter used for the model was  gamma = 0 and the accuracy reached was 64.95%.
...
the ROC curve was plotted below and shows the relationship between sensitivity and specificity. Classifiers that provide curves closer to the upper left corner have the corresponding area under the larger curves (AUCs) and indicate better performance. Multi-class area under the curve: 83.11%
...
The predictive models performed previous were evalutated based on the accuracy measurement that calculate the proportion of the total number of predictions that were correct. In order to evaluate the performace of models it was used the `DALEX` R package.

DALEX contains a set of tools that help to provide *D*escriptive m*A*chine Learning *EX*planations ranging from global to local interpretability methods. In particular, it makes comparing performance across multiple models convenient.

Below can be a rank of the model performance by accuracy:

```{r resample_models, eval=FALSE, include=FALSE}
...
```{r eval_models, eval=FALSE, include=FALSE}
...
In this work we start from an informed data set, we did some processes for cleaning, imputing and transforming the data. It was necessary to reduce the dimensionality of the data set and include some important variables that were taken from stations that record meteorological measurements and a selection of variables that would be important for the prediction of the cause of forest fires was carried out.
We tested 13 models for the treatment of multiclass response. As a performance analysis, one of the main elements observed in the survey, was the accuracy, which is one of the determining characteristics in the evaluation of a predictor. Another information that we took into account was the plotting of the ROC curve that shows the relationship between sensitivity and sensitivity and, informs that the closer the curve gets to the upper left corner of the graph, presenting a larger area under it, it indicates the best performance for the model.
In the analysis, we had models with performance ranging from 55.49% to 64.38%, with XGBoost as the best performance model and adding to this, the value of 83.11% achieved based on the ROC curve for the same model, exceeding the values of the others tested.
The improvement of the data sets and the processing of it in an appropriate way guarantees a better result and the attempt to achieve a good performance.
...
http://www.sthda.com/english/articles/36-classification-methods-essentials/149-penalized-logistic-regression-essentials-in-r-ridge-lasso-and-elastic-net/
 ...
 Pang Ning, 2006, Introduction to Data Mining