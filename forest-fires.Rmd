...
The lat variable had to be corrected because we observed that there were incorrect values like a date mixed in it.
```{r echo=FALSE}
...
In the wrong values, an imputation was made based on another observation that has the same region, district, municipality and parish than these. After the imputation 8 NA values were assigned to observations.
```{r echo=FALSE}
...
It was necessary to do a cleaning on lat and lon variables and convert their contents from GPS coodinate to decimals.Before the transformation they were like below:

first lines of lat variable
```{r echo=FALSE}
head(fires.raw$lat)
```
first lines of lon variable
```{r echo=FALSE}
head(fires.raw$lon)
```
...
fires.raw <- fires.raw %>% rowwise() %>% mutate(firstInterv_date = if_else( is.na(firstInterv_date) && !is.na(extinction_date), extinction_date , firstInterv_date),firstInterv_hour = if_else( is.na(firstInterv_hour) && !is.na(extinction_hour), extinction_hour , firstInterv_hour))
```
